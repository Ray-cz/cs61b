...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            long id = Long.parseLong(attributes.getValue("id"));
            currWay = new GraphDB.Way(id);
            g.addWay(currWay);
            wayNodes = new ArrayList<>();
        } else if (activeState.equals("way") && qName.equals("nd")) {
            /* While looking at a way, we found a <nd...> tag. */
            //System.out.println("Id of a node in this way: " + attributes.getValue("ref"));
            long refNode = Long.parseLong(attributes.getValue("ref"));
            wayNodes.add(refNode);
            g.getNodes().get(refNode).wayOn.add(currWay.id);
            ...
            if (k.equals("maxspeed")) {
                ...
                if (ALLOWED_HIGHWAY_TYPES.contains(v)) {
                    for (int i = 1; i < wayNodes.size(); i++) {
                        Long prev = wayNodes.get(i - 1);
                        Long curr = wayNodes.get(i);
                        g.addAdj(prev, curr);
                        g.addAdj(curr, prev);
                    }
                }
